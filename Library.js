...
class Library {
  ...
  addBook(book) {
    if (book.title && book.author) {
      ...
      console.error("Ein Buch benötigt mindestens einen Titel und einen Autor.");
    }
  }

  findTitle(title) {
    return this._books.filter(book => book.title.toLowerCase() === title.toLowerCase());
  }
  ...
  removeTitle(title) {
    this._books = this._books.filter(book => book.title.toLowerCase() !== title.toLowerCase());
  }

  findByGenre(genre) {
    return this._books.filter(book => book.genre.toLowerCase() === genre.toLowerCase());
  }
}
